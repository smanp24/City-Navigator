...
class City():
    ...
    def distance(self, other_city: City) -> int:
        ...
        """
        distance = round(geopy.distance.great_circle(self.coordinates,other_city.coordinates).km)
        return(distance)
    ...
    def get_table_data(self) -> list[str]:
        ...
        table_data.append(f'{self.coordinates}')
        table_data.append(self.city_type)
        table_data.append(f'{self.population}')
        table_data.append(f'{self.city_id}')
...
def get_city_by_id(city_id: int) -> City | None:
    ...
    """
    for instance in City.id_to_cities:
        if city_id == instance:
            return City.id_to_cities[instance]
...
def get_cities_by_name(city_name: str) -> list[City]:
    ...
    """
    for instance in City.name_to_cities:
        if city_name == instance:
            City.name_to_cities[instance].append(city_name)
    return City.name_to_cities[instance]
...
if __name__ == "__main__":
    ...
    #test_example_cities()
    City('Melbourne', (-37.8136, 144.9631), 'primary', 1000000, 1)
    City('Melbourne', (-33.865143, 151.209900), 'admin', 5000000, 2)
    print(get_cities_by_name('Melbourne'))
